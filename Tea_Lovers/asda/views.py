    ...
    if request.method == 'POST':
        try:
            ...
            product, _ = Product.objects.get_or_create(
                store=store, name=name, defaults={'price': price})
            basket, _ = Basket.objects.get_or_create(user=request.user)

            # Add product to the basket
            basket_item = BasketItem.objects.create(
                basket=basket, product=product)
            logger.info(f'item added to basket: {basket_item.product.name}, ID:{basket_item.id}')
...
def remove_from_basket(request):
    ...
    try:
        ...
        logger.info(f'item removed from basket: {item_id}')
        # Remove the item from the basket database
        item.delete()  
